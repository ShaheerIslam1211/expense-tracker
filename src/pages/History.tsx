...
import { HistoryFiltersForm, HistoryInsightsColumn, type DatePresetId, type SortId } from "./history/HistoryPanels";
  ...
  return (
    ...
    >
      <div className="flex items-start sm:items-center justify-between gap-3 sm:gap-5">
        <div className="flex min-w-0 flex-1 items-start gap-2 sm:gap-4">
          ...
          <div className="min-w-0 flex-1 pr-1">
            <div className="flex items-center gap-2 flex-wrap">
              <p className="font-bold text-foreground text-sm sm:text-base line-clamp-2 sm:truncate wrap-break-word">
        ...
        <div className="flex max-w-[42%] shrink-0 flex-col items-end gap-1 text-right sm:max-w-48 md:max-w-52 sm:gap-2">
          <p
            className={cn(
...
export default function History() {
  ...
  const filteredAndSorted = useMemo(() => {
    ...
    const catName = (id: string) => categories.find((c) => c.id === normalizeCategoryId(id))?.name ?? id;
      ...
      <div className="grid grid-cols-3 md:grid-cols-2 lg:grid-cols-6 gap-1.5 sm:gap-2 md:gap-3">
        {[
          ...
          { label: "Expenses", value: totalExpenseAmount, className: "text-foreground", prefix: "" },
          {
            ...
            className: netAmount >= 0 ? "text-success" : "text-danger",
            prefix: netAmount >= 0 ? "+" : "−",
          },
          {
            label: "Transactions",
          ...
          { label: "Avg size", value: averageAmount, className: "text-foreground", prefix: "" },
          {
            ...
            prefix: "",
            sub: `${expenseCount} exp · ${incomeCount} inc`,
          },
        ].map((kpi, i) => (
          <div
            key={i}
          ...
          >
            ...
            {kpi.sub ? (
              <p className="text-xs sm:text-sm font-black text-foreground mt-0.5 sm:mt-1 leading-tight wrap-break-word">
                {kpi.sub}
              </p>
            ) : (
              <p
        ...
        <div className="flex flex-wrap items-center gap-2">
          ...
          <label className="flex items-center gap-1.5 text-[10px] font-black text-muted-foreground uppercase shrink-0">
            ...
            <select
              ...
              className="px-2 py-1.5 rounded-lg bg-accent/10 border border-border font-bold min-w-14"
            >
          ...
          <>
            {paginated.map((expense, i) => {
              const showDayHeader =
                groupByDay &&
                (i === 0 ||
                  format(parseISO(paginated[i - 1].date), "yyyy-MM-dd") !==
            ...
            {totalPages > 1 && (
              <div className="flex flex-wrap items-center justify-center gap-3 pt-4">
                ...
                <span className="text-xs font-bold text-muted-foreground tabular-nums">
                  {pageStart + 1}–{Math.min(pageStart + pageSize, filteredAndSorted.length)} of{" "}
                      ...
                      <div className="space-y-3 text-sm">
                        <div>
                          ...
                          <p className="font-bold text-foreground">
                            {format(parseISO(photoModalExpense.date), "dd MMM yyyy")}
                          </p>
                        </div>
                        <div>
                          <p className="text-[10px] font-black uppercase tracking-widest text-muted-foreground">
                          ...
                          <p className="font-bold text-foreground">
                            {categories.find((c) => c.id === normalizeCategoryId(photoModalExpense.categoryId))?.name ||
