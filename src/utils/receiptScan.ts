...
function parseAmountFromEnd(line: string): number | null {
  ...
  const m =
    trimmed.match(/(\d{1,3}(?:,\d{3})*(?:\.\d{1,2})?)\s*$/) ??
    trimmed.match(/(\d+(?:\.\d{1,2})?)\s*(?:Rs\.?|PKR)?\s*$/i);
  if (m) {
...
function extractMerchant(lines: string[]): string | undefined {
  ...
  for (let i = 0; i < Math.min(10, lines.length); i++) {
    ...
    if (/opp\.|cantt|airport|phase|road|street|rd\.|st\.|walking|cash\s*sales/i.test(line) && line.length < 35)
      continue;
  ...
  for (let i = 0; i < Math.min(6, lines.length); i++) {
    ...
    if (alphaCount >= 3 && symbolCount <= line.length * 0.2) {
      return line.replace(/1/g, "l").replace(/0/g, "o").replace(/\s+/g, " ").trim();
    }
  }
